...
При стартиране `data.sql` зарежда два вида кредити, ако не съществуват:
...
Допълнително `DatabaseSeeder` добавя примерни клиенти, сметки, кредити и погасителни планове за демонстрация на функционалностите. Seed-ването е идемпотентно и може да се изключи чрез:
...
Вноските се плащат последователно. Ако има неплатена по-ранна вноска, системата не позволява плащане на по-късен месец и връща `BusinessRuleException` (HTTP 422).

Плащането изисква `accountId` в request body. Системата проверява, че сметката:
- съществува;
- е активна;
- принадлежи на клиента по кредита;
- има достатъчна наличност за `totalPayment` на вноската.

При успешно плащане балансът на сметката се намалява със сумата на вноската и вноската се маркира като платена в една транзакция.
...
|---|---|---|
| React | 18 | UI рамка |
| TypeScript | 4.9.x | Типизиран JavaScript, съвместим с Create React App 5 |
...
| `LoanServiceTest` | Отпускане, валидация на лимити, последователно плащане, статус, промяна на вид |
...
| `LoanControllerIT` | Всички кредитни endpoints, погасителен план, последователно плащане, статус |
